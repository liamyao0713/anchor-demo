...
assert.match(html, /data-about-lang="zh"/, "About section must support Chinese static copy");
assert.doesNotMatch(html, /id="aw-about-lang-en" class="aw-mode-button/, "About language buttons must not reuse the corrected-answer mode class");
...
assert.match(workspaceUiJs, /ABOUT_LANGUAGE_STORAGE_KEY/, "workspace must persist the static About language setting");
assert.match(workspaceUiJs, /setAboutLanguage/, "workspace must wire About language switching");
